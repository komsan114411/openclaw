...
async def api_status_check():
    ...
    if thunder_token:
        ...
        try:
            # เปลี่ยนจากการเรียก /v1/user ไปเป็นการทดสอบปลายทาง verify/truewallet ด้วยข้อมูลจำลอง
            ...
            if response.status_code == 400:
                # API ตอบสนองด้วยข้อผิดพลาดที่คาดหวังสำหรับข้อมูลจำลอง = เชื่อมต่อสำเร็จ
...
async def test_thunder_api():
    ...
    try:
        ...
        # ใช้ปลายทางที่ถูกต้องสำหรับการทดสอบ เช่น /v1/user ไม่ได้มีในเอกสาร
        # ลองใช้ปลายทาง verify/truewallet กับข้อมูลจำลองเพื่อยืนยันการเชื่อมต่อ
        dummy_file = {"file": ("dummy.txt", b"not_an_image", "text/plain")}
        response = requests.post("https://api.thunder.in.th/v1/verify/truewallet",
        ...
        if response.status_code == 400:
            ...
            return JSONResponse(content={"status": "success", "message": "เชื่อมต่อ Thunder API สำเร็จ (ตอบกลับ 200 OK)", "user": response.json().get("name", "Unknown"), "balance": response.json().get("balance", 0)})
    except Exception as e:

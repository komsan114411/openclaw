...
async def safe_import_modules():
    ...
    global IS_READY, config_manager, database_functions, ai_functions, slip_functions, line_account_manager, db_manager
    ...
    try:
        ...
        try:
            from models.database import (
                MongoDBManager, init_database, save_chat_history, get_chat_history_count, 
                ...
                get_recent_chat_history_by_account, get_user_info_for_admin,
                db_manager as db_m,
            )
            ...
            if init_result:
                ...
                logger.info("✅ LineAccountManager initialized and indexes created")
                ...
                database_functions = {
                    ...
                    'get_recent_chat_history_by_account': get_recent_chat_history_by_account,
                    'get_user_info_for_admin': get_user_info_for_admin,
                }
        ...
        if not database_import_success:
            ...
            async def dummy_save(u, d, m, s, a=None):
            ...
            async def dummy_recent_by_account(a, l=50):
                return []
            ...
            database_functions = {
                ...
                'save_chat_history_with_account': dummy_save,
                'get_recent_chat_history_by_account': dummy_recent_by_account,
                'get_user_info_for_admin': lambda u: {},
            }
...
async def get_chat_messages_api(limit: int = 100, account_id: Optional[str] = None):
    """API endpoint สำหรับดึงข้อความแชททั้งหมด"""
    try:
        messages = []
        
        if database_functions and 'get_recent_chat_history_by_account' in database_functions and account_id:
            history = await database_functions['get_recent_chat_history_by_account'](account_id, limit)
            messages = [
                {
                    "id": str(doc.get('_id', '')),
        ...
        elif database_functions and 'get_recent_chat_history' in database_functions:
...
async def line_webhook_multi_account(request: Request, background_tasks: BackgroundTasks, 
                                     account_id: str, x_line_signature: str = Header(None)) -> JSONResponse:
    """LINE webhook endpoint for multiple accounts"""
    if not IS_READY or not line_account_manager:
        return JSONResponse(content={"status": "error", "message": "System not ready"}, status_code=503)
    ...
    try:
        ...
        # Verify signature
        line_secret = account.get("channel_secret")
        hash_signature = hmac.new(line_secret.encode('utf-8'), body, hashlib.sha256).digest()
        if not hmac.compare_digest(x_line_signature.encode('utf-8'), base64.b64encode(hash_signature)):
...
async def get_user_chat_history_api(user_id: str, request: Request):
    ...
    try:
        ...
        chat_history = []
        if database_functions and 'get_user_chat_history_for_admin' in database_functions:
            # ดึงข้อความทั้งหมดของ user นี้
            chat_history = await database_functions['get_user_chat_history_for_admin'](user_id, account_id=None, limit=100)
        
        # ดึงข้อมูล user
        user_info = {}
        if database_functions and 'get_user_info_for_admin' in database_functions:
            user_info = await database_functions['get_user_info_for_admin'](user_id)

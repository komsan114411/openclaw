...
def safe_import_modules():
    ...
    try:
        ...
        # Initialize database WITH FORCE TABLE CREATION
        try:
            ...
            try:
                ...
                logger.info("✅ Database tables force created")
            except Exception as table_error:
                logger.warning(f"⚠️ Force table creation failed: {table_error}")
                # Try SQLite fallback
                try:
                    from models.database import init_database as sqlite_init
                    sqlite_init()
                    logger.info("✅ SQLite tables created as fallback")
        ...
        try:
            from models.postgres_models import db_manager
            db_type = "PostgreSQL" if 'postgres' in str(db_manager.engine.url) else "SQLite"
        except:
            db_type = "SQLite"
        ...
        class DummyConfigManager:
            def __init__(self):
                self.config = {
                    ...
                    'thunder_api_token': os.environ.get('THUNDER_API_TOKEN', ''),
        ...
        logger.info("⚠️ Using fallback dummy config manager")
...
async def test_thunder_connection():
    """ทดสอบการเชื่อมต่อ Thunder API with better error handling"""
    try:
        ...
        if result.get("status") == "success":
            await notification_manager.send_notification("Thunder API ทดสอบสำเร็จ", "success")
        else:
            await notification_manager.send_notification(f"Thunder API Error: {result.get('message')}", "warning")
        ...
        await notification_manager.send_notification(f"Thunder API Test Error: {str(e)}", "error")
        return JSONResponse({
            "status": "error",
            "message": f"เกิดข้อผิดพลาด: {str(e)}"
        })
        @app.post("/admin/force-init-database")
async def force_init_database():
    ...
    try:
        ...
        try:
            from models.postgres_models import db_manager
            db_manager.create_tables()
            await notification_manager.send_notification("PostgreSQL tables created successfully", "success")
            return JSONResponse({
                "status": "success",
            ...
            logger.warning(f"PostgreSQL init failed: {pg_error}")
            
            # Fallback to SQLite
            from models.database import init_database
            init_database()
            await notification_manager.send_notification("SQLite database initialized successfully", "success")
            return JSONResponse({
                "status": "success",
        ...
        logger.error(f"❌ Force database init error: {e}")
        await notification_manager.send_notification(f"Database init error: {str(e)}", "error")
        return JSONResponse({
            "status": "error",
            "message": f"เกิดข้อผิดพลาด: {str(e)}"
        })

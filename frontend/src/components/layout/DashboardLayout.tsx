...
export default function DashboardLayout({ children, requiredRole }: DashboardLayoutProps) {
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans selection:bg-emerald-500/30 bg-[#0A0F0D]">
      ...
      {/* Mobile Glass Header */}
      <div className="md:hidden fixed top-0 inset-x-0 h-14 sm:h-16 glass-navbar z-30 flex items-center px-3 sm:px-4 justify-between safe-top">
        <motion.button
          whileTap={{ scale: 0.9 }}
          onClick={() => setIsSidebarOpen(true)}
          className="p-2.5 min-h-[44px] min-w-[44px] bg-white/5 rounded-xl border border-white/10 shadow-lg text-white flex items-center justify-center hover:bg-white/10 transition-all active:bg-white/15 touch-manipulation"
          aria-label="เปิดเมนู"
        >
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden relative z-10 w-full bg-[#0A0F0D] safe-bottom">
        {/* Mobile Header Spacer */}
        <div className="md:hidden h-14 sm:h-16" />

        <div className="container-responsive w-full animate-fade px-3 sm:px-4 md:px-6 pb-6 sm:pb-10">
          {children}
        </div>
      </main>
    </div>

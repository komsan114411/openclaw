...
class LineAccountManager:
    ...
    async def _ensure_indexes(self):
        ...
        try:
            ...
            logger.info("✅ Indexes created for line_accounts collection")
    ...
    async def list_accounts(self) -> List[Dict[str, Any]]:
        ...
        try:
            # ใช้ cursor อย่างถูกต้องกับ async
            cursor = self.accounts_collection.find({}).sort("updated_at", -1)
            accounts = []
            
            # ใช้ async for กับ cursor
            async for doc in cursor:
                accounts.append(self._serialize(doc))
    ...
    async def create_account(self, data: Dict[str, Any]) -> str:
        ...
        account_data = {
            # Display / meta
            "display_name": self._clean_str(data.get("display_name"), "Unnamed Account"),
        ...
        try:
            ...
            logger.info(f"✅ Created new account: {account_data['display_name']}")
            return str(res.inserted_id)
        except DuplicateKeyError:
            # webhook_path ชนกัน → สุ่มใหม่แล้วลองอีกครั้ง
            account_data["webhook_path"] = self._generate_webhook_path()
            res = await self.accounts_collection.insert_one(account_data)
            logger.info(f"✅ Created new account (retry): {account_data['display_name']}")
            return str(res.inserted_id)
        except Exception as e:
    ...
    async def update_account(self, account_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Text fields
            text_fields = [
                "display_name", "description", "channel_secret", "channel_access_token",
                "thunder_api_token", "openai_api_key", "kbank_consumer_id", 
                "kbank_consumer_secret", "ai_prompt"
            ]
            
            for field in text_fields:
                if field in updates:
                    cleaned[field] = self._clean_str(updates[field])
            
            # System messages with defaults
            message_fields = {
                "ai_disabled_message": "ขออภัย ระบบ AI ถูกปิดการใช้งานชั่วคราว",
                "slip_disabled_message": "ขออภัย ระบบตรวจสอบสลิปถูกปิดการใช้งานชั่วคราว",
                "system_disabled_message": "ขออภัย ระบบกำลังปิดปรับปรุง กรุณาติดต่อใหม่ภายหลัง"
            }
            
            for field, default in message_fields.items():
                if field in updates:
                    cleaned[field] = self._clean_str(updates[field], default)
            ...
            if "webhook_path" in updates:
                cleaned["webhook_path"] = self._clean_str(updates.get("webhook_path")) or self._generate_webhook_path()
            ...
            if res.modified_count > 0:
                ...
                logger.warning(f"⚠️ No changes made to account: {account_id}")
                return False
    ...
    async def get_account_stats(self, account_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # หา last activity
            last_message = await self.db.chat_history.find_one(
            ...
            return {"total_messages": 0, "last_activity": None}

    # ------------------------- Test Connection -------------------------

    async def test_account_connection(self, account_id: str) -> Dict[str, Any]:
        """ทดสอบการเชื่อมต่อ LINE API ของบัญชี"""
        try:
